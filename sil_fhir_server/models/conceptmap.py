...
# -*- coding: utf-8 -*-
#
#  FHIR 1.0.2.7202 (http://hl7.org/fhir/StructureDefinition/ConceptMap)
#  Date: 2016-03-22.
...
class ConceptMapElementTarget(backboneelement.BackboneElement):
    ...
    dependsOn = Column(primitives.StringField,
                       ForeignKey(
                           'ConceptMapElementTargetDependsOn.id'))
    """ Other elements required for this mapping (from context).
        List of `ConceptMapElementTargetDependsOn` items
...
class ConceptMapElement(backboneelement.BackboneElement):
    ...
    target = Column(primitives.StringField,
                    ForeignKey('ConceptMapElementTarget.id'))
    """ Concept in target system for element.
...
class ConceptMap(domainresource.DomainResource):
    ...
    contact = Column(primitives.StringField,
                     ForeignKey('ConceptMapContact.id'))
    """ Contact details of the publisher.
    ...
    element = Column(primitives.StringField,
                     ForeignKey('ConceptMapElement.id'))
    """ Mappings for a concept from the source set.
    ...
    # todo sourceReference = Column(primitives.StringField, ForeignKey('FHIRReference.id'))
    sourceReference = Column(primitives.StringField)
    ...
    # todo targetReference = Column(primitives.StringField, ForeignKey('FHIRReference.id'))
    targetReference = Column(primitives.StringField)
    """ Provides context to the mappings.
        Type `FHIRReference` referencing `ValueSet, StructureDefinition`
        (represented as `dict` in JSON). """
    ...
    def __init__(self, contact, copyright, date, description, element,
                 experimental, identifier, name, publisher, requirements,
                 sourceReference, sourceUri, status, targetReference,
                 targetUri, url, useContext, version,):
        """ Initialize all valid properties.
        """
